import React from "react";
import Moment from "react-moment";

type Props = {
  reciever?: boolean;
  name: string;
  message: string;
  timestamp: Date;
};

function Message({ reciever, name, message, timestamp }: Props) {
  return (
    <>
      {reciever ? (
        <div className="flex justify-end">
          <div className="max-w-[500px] ">
            <div className="mb-2 text-right text-[12px] font-semibold">
              {name}
            </div>
            <div className="flex items-center">
              <div className="relative mr-3 min-h-[66px] min-w-[180px] max-w-[400px] break-words rounded-tl-[14px] rounded-br-[14px] rounded-bl-[14px] bg-gray-300 bg-opacity-40 py-4 px-3 text-right text-black">
                {message}
                <span className="absolute bottom-2 left-4 text-[10px] text-[#96A9BA]">
                  <Moment format="mm:ss">{timestamp}</Moment>
                </span>{" "}
              </div>
              <div className="relative h-[44px] w-[44px] rounded-full border-[2px] border-[#F3BA4A]">
                <img
                  className="rounded-full"
                  src="data:image/png;base64,"
                />
              </div>
            </div>
          </div>
        </div>
      ) : (
        <div>
          <div className="max-w-[500px]">
            <div className="mb-2 text-[12px] font-semibold">{name}</div>
            <div className="flex items-center">
              <div className="relative h-[44px] w-[44px] rounded-full border-[2px] border-[#F3BA4A] ">
                <img
                  className="rounded-full "
                  src="data:image/png;base64,"
                />
              </div>
              <div className="relative ml-3 min-h-[66px] min-w-[180px] max-w-[400px] break-words rounded-tr-[14px] rounded-br-[14px] rounded-bl-[14px] bg-opacity-30 bg-indigo-300 py-4 px-3">
                {message}
                <span className="absolute bottom-2 right-4 text-[10px] text-[#96A9BA]">
                  <Moment format="mm:ss">{timestamp}</Moment>
                </span>
              </div>
            </div>
          </div>
        </div>
      )}
    </>
  );
}

export default Message;
